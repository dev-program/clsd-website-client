...
const EventList = () => {
  ...
  const availableTags = [...new Set(tutorials.flatMap((tutorial) => tutorial.identification))];
...
const retrieveTutorials = (year, identification) => {
  UserService.getAll(year, identification)
    .then((response) => {
      let filteredTutorials = response.data;
      if (year) {
      ...
      if (identification) {
        filteredTutorials = filteredTutorials.filter(
          (tutorial) => tutorial.identification.includes(identification)
        );
      }
      setTutorials(filteredTutorials);
      console.log(response.data);
...
const handleDelete = () => {
  ...
  const handleTagFilter = (identification) => {
    setSelectedTag(identification);
    retrieveTutorials(selectedYear, identification);
  };
...
<div className="tag-filter">
  ...
  {availableTags.map((identification) => (
    <button
      className={`m-3 btn btn-sm btn-info ${selectedTag === identification ? "active" : ""}`}
      onClick={() => handleTagFilter(identification)}
      key={identification}
    >
      {identification}
    </button>
...
<div className="d-flex justify-content-between align-items-center">
  ...
  {(showAdminBoard || showModeratorBoard) && currentUser && (
    <div>
      <Link
        to={"/publication/" + currentTutorial.id}
      ...
      >
        Edit Publication
                ...
                <div className= 'mt-4'>
                <label>Created:</label> 
                {new Date(currentTutorial.createdAt).toLocaleDateString("en-US",options)}     <label>Updated:</label>  {new Date(currentTutorial.updatedAt).toLocaleDateString("en-US",options)}
                </div>
