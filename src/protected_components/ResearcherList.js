...
const ResearcherList = () => {
  ...
  const availableTags = [...new Set(tutorials.flatMap((tutorial) => tutorial.researchinterest))];
  ...
  const handleExportCSV = () => {
    ...
    const dataToExport = selectedTutorials.map((tutorial) => ({
      ...
      middleName: tutorial.middleName,
      lastName: tutorial.lastName,

      suffix: tutorial.suffix,
    
      researchinterest: tutorial.researchinterest,
...
const retrieveTutorials = (year, tag) => {
  UserService.getAll(year, tag)
    .then((response) => {
      ...
      if (tag) {
        filteredTutorials = filteredTutorials.filter(
          (tutorial) => tutorial.researchinterest.includes(tag)
        );
      }
      setTutorials(filteredTutorials);
      console.log(response.data);
...
const options = { month: 'long', day: 'numeric' , year: 'numeric'};
  ...
  <ol class="breadcrumb">
    <li class="breadcrumb-item"><a >Researchers</a></li>
...
<div className="d-flex justify-content-between align-items-center">
  ...
  <div>
    <label>
      <strong>Current Researchers:</strong>
    </label>{" "}
  ...
  {(showAdminBoard || showModeratorBoard) && currentUser && (
    <div>
      <Link
        to={"/researcher/" + currentTutorial.id}
